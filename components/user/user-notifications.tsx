...
export function UserNotifications({ className }: UserNotificationsProps) {
  ...
  return (
    <Card className={`border-0 shadow-lg bg-white/50 dark:bg-slate-800/50 backdrop-blur-sm ${className ?? ""}`.trim()}>
      <CardHeader>
      ...
      <CardContent>
        <div className="space-y-3">
          {displayNotifications.map((notification, index) => (
            <div
              key={String(notification._id || notification.id || `${notification.createdAt || "n"}-${index}`)}
